	...
	class Redux_Border extends Redux_Field {
		...
		public function render() {
			...
			if ( false !== $this->field['style'] ) {
				...
				// Added by TemPlaza
				$styles     = isset($this -> field['styles'])?$this -> field['styles']:array();
                $options    = array_merge($styles, $options);

				$placeholder    = isset($this -> field['placeholder'])?$this -> field['placeholder']:array();
//				if(!empty($placeholder) && is_array($placeholder) && isset($placeholder['style'])){
//				    $options    = array_merge(array('' => $placeholder['style']), $options);
//                }
				$placeholder_style  = is_array($placeholder) && isset($placeholder['style'])?$placeholder['style']:esc_html__( 'Border style', 'redux-framework' );

				$select2_data_tags  = isset($this->field['select2']['tags'])?filter_var($this->field['select2']['tags'],
                    FILTER_VALIDATE_BOOLEAN):false;

				echo '<select data-placeholder="' . $placeholder_style . '" id="' . esc_attr( $this->field['id'] ) . '[border-style]" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[border-style]" class="tips redux-border-style ' . esc_attr( $this->field['class'] ) . '" rows="6" data-id="' . esc_attr( $this->field['id'] ) . '"' . esc_attr( $select2_data ) . '>';
...
//					echo '<option value="' . esc_attr( $k ) . '" ' . selected( $value['style'], $k, false ) . '>' . esc_html( $v ) . '</option>';
					...
					// Added by TemPlaza
                    if($select2_data_tags){
                        if(strlen($k)){
                            echo '<option value="' . esc_attr( $k ) . '" ' . selected( $value['style'], $k, false ) . '>' . esc_html( $v ) . '</option>';
                        }else{
                            echo '<option ' . selected( $value['style'], $k, false ) . '>' . esc_html( $v ) . '</option>';
                        }
                    }else{
                        echo '<option value="' . esc_attr( $k ) . '" ' . selected( $value['style'], $k, false ) . '>' . esc_html( $v ) . '</option>';
                    }
